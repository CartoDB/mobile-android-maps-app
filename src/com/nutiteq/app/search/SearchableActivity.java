	...
	public void onCreate(Bundle bundle) {
		...
		getListView().setOnItemClickListener(
				new AdapterView.OnItemClickListener() {
					...
					@Override
					public void onItemClick(AdapterView<?> aparent, View v,
							int position, long id) {

						// set data for picked search
						Intent data = new Intent();

						data.putExtra(MainActivity.SEARCH_X,
								searchResultPlaces[position].getGeometry()
										.getCenterPos().getX());
						data.putExtra(MainActivity.SEARCH_Y,
								searchResultPlaces[position].getGeometry()
										.getCenterPos().getY());

						if (searchResultPlaces[position]
								.getMetaDataElement("title") != null) {
							data.putExtra(MainActivity.SEARCH_TITLE,
									searchResultPlaces[position]
											.getMetaDataElement("title"));
						}
						if (searchResultPlaces[position]
								.getMetaDataElement("description") != null) {
							data.putExtra(MainActivity.SEARCH_DESC,
									searchResultPlaces[position]
											.getMetaDataElement("description"));
						}
						if (searchResultPlaces[position]
								.getMetaDataElement("x1") != null) {
							data.putExtra("x1", searchResultPlaces[position]
									.getMetaDataElement("x1"));
						}
						if (searchResultPlaces[position]
								.getMetaDataElement("x2") != null) {
							data.putExtra("x2", searchResultPlaces[position]
									.getMetaDataElement("x2"));
						}
						if (searchResultPlaces[position]
								.getMetaDataElement("y1") != null) {
							data.putExtra("y1", searchResultPlaces[position]
									.getMetaDataElement("y1"));
						}
						if (searchResultPlaces[position]
								.getMetaDataElement("y2") != null) {
							data.putExtra("y2", searchResultPlaces[position]
			...
			public void run() {
				...
				try {
					...
					if (geocodeResults != null && !isCancled) {
						...
						MarkerStyle searchMarkerStyle = markerStyleBuilder
								.buildStyle();
						...
						for (int i = 0; i < l; i++) {
							...
							searchResultPlaces[i] = new Marker(proj
									.fromWgs84(new MapPos(
											geocodeResults.get(i).lon,
											geocodeResults.get(i).lat)),
									searchMarkerStyle);
							searchResultPlaces[i].setMetaDataElement("title",
									geocodeResults.get(i).line1);
							searchResultPlaces[i].setMetaDataElement(
									"description", geocodeResults.get(i).line2);

							searchResultPlaces[i].setMetaDataElement("x1",
									geocodeResults.get(i).boundingBox.getMin()
											.getX() + "");
							searchResultPlaces[i].setMetaDataElement("y1",
									geocodeResults.get(i).boundingBox.getMin()
											.getY() + "");
							searchResultPlaces[i].setMetaDataElement("x2",
									geocodeResults.get(i).boundingBox.getMax()
											.getX() + "");
							searchResultPlaces[i].setMetaDataElement("y2",
									geocodeResults.get(i).boundingBox.getMax()
											.getY() + "");
						}
	...
	final Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			SimpleAdapter listAdapter = new SimpleAdapter(
					SearchableActivity.this, list, R.layout.searchrow,
