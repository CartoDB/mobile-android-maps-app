...
public class SettingsFragment extends PreferenceFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.preferences11);

        final ListPreference lang = (ListPreference) findPreference(Const.PREF_LANG_KEY);

        lang.setSummary(getRealLangName(lang.getValue()));
        ...
        lang.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference,
                ...
                lang.setSummary(getRealLangName(object.toString()));

                return true;
            }
        });

        final ListPreference unit = (ListPreference) findPreference(Const.PREF_UNIT_KEY);

        unit.setSummary(getRealUnitName(unit.getValue()));
        ...
        unit.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference,
                ...
                unit.setSummary(getRealUnitName(object.toString()));

                int m = 0;
                if (object.toString().equals(Const.METRIC)) {
                    m = LocationTrackingDB.METRIC;
                } else if (object.toString().equals(Const.IMPERIAL)) {
                    m = LocationTrackingDB.IMPERIAL;
                }

                ((MapApplication) getActivity().getApplication())
                        .getLocationTrackingDB().setMeasurementUnit(m);

                return true;
            }
        });

        storage = (ListPreference) findPreference(Const.PREF_STORAGE_KEY);
        ...
        setStorageSummary(storageValue);
            ...
            public boolean onPreferenceChange(Preference preference,
                ...
                if (storageValue.equals(Const.INTERNAL)) {
                    ...
                    if (lastStorageValue.equals(Const.EXTERNAL)) {
                        ...
                        try {
                            parse = Integer.parseInt(lastStorageValue
                                    .substring(lastStorageValue
                                            .indexOf(" ") + 1));
                        } catch (NumberFormatException e) {
                        }

                        File[] f = new File(files[parse - 1],
                    ...
                    if (storageValue.equals(Const.INTERNAL)) {
                        ...
                        try {
                            parse = Integer.parseInt(storageValue
                                    .substring(storageValue
                                            .indexOf(" ") + 1));
                        } catch (NumberFormatException e) {
                        }
                ...
                setStorageSummary(object.toString());

                return true;
            }
        });

        final CheckBoxPreference buildings3D = (CheckBoxPreference) findPreference(Const.PREF_BUILDINGS_3D_KEY);

        buildings3D.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object object) {
                MainActivity.setShouldUpdateBaseLayer(true);

                if (object.toString().equals("true")) {
                    MainActivity.setBuildings3D(true);
                } else if (object.toString().equals("false")) {
                    MainActivity.setBuildings3D(false);
                }
    ...
    private void setFreeStorageInfo() {
        ...
        if (activity != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ...
                if (storage.getValue().equals(Const.EXTERNAL)) {
                    storage.setValue(Const.EXTERNAL + " 1");
                }
            } else {
                String external = "";
    ...
    private void setStorageSummary(String s) {
        if (s.toString().contains(Const.EXTERNAL)) {
            if (storage.getEntries().length > 2) {
                String str = "";

                try {
                    str = Integer.parseInt(s.substring(s.indexOf(" ") + 1)) + "";
                } catch (NumberFormatException e) {
                }

                storage.setSummary(getString(R.string.external) + " "
                        + str);
            } else {
                storage.setSummary(getString(R.string.external));
            }
        } else {
            storage.setSummary(getString(R.string.internal));
        }
    }
}
